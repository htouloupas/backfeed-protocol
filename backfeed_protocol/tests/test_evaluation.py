...
class EvaluationTest(BaseContractTestCase):
    ...
    def test_get_evaluation(self):
        ...
        evaluation = self.contract.create_evaluation(
            contribution=contribution, user=user, value=value)
    ...
    def test_get_evaluations(self):
        ...
        self.assertEqual(len(contract.get_evaluations(value=0)), 1)
        self.assertEqual(len(contract.get_evaluations(
            contribution_id=self.contribution0.id)), 3)
        self.assertEqual(len(contract.get_evaluations(
            contribution_id=self.contribution1.id)), 2)
        self.assertEqual(len(contract.get_evaluations(
            contribution_id=self.contribution2.id)), 0)
        self.assertEqual(
            len(contract.get_evaluations(evaluator_id=self.user0.id)), 0)
        self.assertEqual(
            len(contract.get_evaluations(evaluator_id=self.user1.id)), 1)
        self.assertEqual(
            len(contract.get_evaluations(evaluator_id=self.user2.id)), 2)
        self.assertEqual(
            len(contract.get_evaluations(evaluator_id=self.user3.id)), 2)
    ...
    def test_token_rewards_for_evaluators(self):
        ...
        self.assertEqual(user1.tokens, contract.USER_INITIAL_TOKENS + 0.2 *
                         contract.CONTRIBUTION_TYPE[contribution0.contribution_type]['token_fund_for_evaluators'])
        contract.create_evaluation(user2, contribution0, value=0)
        self.assertEqual(user2.tokens, contract.USER_INITIAL_TOKENS + 0.2 *
                         contract.CONTRIBUTION_TYPE[contribution0.contribution_type]['token_fund_for_evaluators'])
        contract.create_evaluation(user3, contribution0, value=1)
        self.assertEqual(user3.tokens, contract.USER_INITIAL_TOKENS + 0.2 *
                         contract.CONTRIBUTION_TYPE[contribution0.contribution_type]['token_fund_for_evaluators'])
        contract.create_evaluation(user4, contribution0, value=0)
        self.assertEqual(user4.tokens, contract.USER_INITIAL_TOKENS + 0.2 *
                         contract.CONTRIBUTION_TYPE[contribution0.contribution_type]['token_fund_for_evaluators'])
    ...
    def test_vote_changing_fee(self):
        ...
        contract.create_evaluation(
            contribution=contribution, user=user, value=1)
        fee_paid = initial_reputation - user.reputation

        contract.create_evaluation(
            contribution=contribution, user=user, value=0)
    ...
    def test_vote_changing_removes_previous_evaluations(self):
        ...
        contribution0 = contract.create_contribution(user=user0)
        contract.create_evaluation(
            contribution=contribution0, user=user0, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user0, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user0, value=0)
    ...
    def test_vote_changing_attack1(self):
        ...
        contract.create_evaluation(
            contribution=contribution0, user=user1, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user2, value=1)

        old_stats = contribution0.get_statistics()

        # we have user1 change his vote to 0, and then to 1 again
        contract.create_evaluation(
            contribution=contribution0, user=user1, value=0)
        contract.create_evaluation(
            contribution=contribution0, user=user1, value=1)
        ...
        self.assertGreater(
            new_stats['engaged_reputation_normal'], old_stats['engaged_reputation_normal'])
    ...
    def test_vote_changing_score_attack2(self):
        ...
        contract.create_evaluation(
            contribution=contribution0, user=user2, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user3, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user4, value=1)
        ...
        contract.create_evaluation(
            contribution=contribution0, user=user1, value=0)
        contract.create_evaluation(
            contribution=contribution0, user=user2, value=0)
        contract.create_evaluation(
            contribution=contribution0, user=user1, value=1)
        contract.create_evaluation(
            contribution=contribution0, user=user2, value=1)
